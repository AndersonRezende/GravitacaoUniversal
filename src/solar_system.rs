pub mod solar_system {
    ...
    */
    pub fn create_first_five() -> Vec<Body> {
        let sun = Body::new(900.0, 500.0, 1989.0000, 0.0, 139.20, 0.0, 0.0, Color::YELLOW);
        let mercury = Body::new(693.0, 500.0, 0.003, 0.0, 4.8, 0.0, 16.0, Color::GRAY);
        let venus = Body::new(542.0, 500.0, 0.048, 0.0, 12.1, 0.0, 22.0, Color::from_rgb(1.0, 0.6, 0.0));
        let earth = Body::new(401.0, 500.0, 0.059, 0.0, 12.7, 0.0, 24.0, Color::BLUE);
        let mars = Body::new(100.0, 500.0, 0.006, 0.0, 6.7, 0.0, 32.0, Color::RED);
        vec![sun, mercury, venus, earth, mars]
    }

    pub fn create_first_four_with_proportion(proportion: i32) -> Vec<Body> {
        let sun = Body::new(950.0, 500.0, 1989.1000, 0.0, 139.20, 0.0, 0.0, Color::YELLOW);
        let mercury = Body::new(743.0, 343.0, 0.3, 0.0, 4.8, -30.0, 0.0, Color::GRAY);
        let venus = Body::new(692.0, 292.0, 4.8, 0.0, 12.1, -30.0, 0.0, Color::from_rgb(1.0, 0.6, 0.0));
        let earth = Body::new(1149.0, 749.0, 5.9, 0.0, 12.7, 30.0, 0.0, Color::BLUE);
        let mars = Body::new(1297.0, 827.0, 0.6, 0.0, 6.7, 30.0, 1.3, Color::RED);
